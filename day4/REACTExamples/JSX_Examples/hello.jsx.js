...
function Employee() {
	...
	this.ename = "";
	this.salary = 0;
	console.log("Default constructor of Employee is called");
	this.empId = 0;
	this.ename = "";
	this.salary = 0;
};

function Employee$0(id, name, sal) {
	this.empId = 0;
	this.ename = "";
	this.salary = 0;
	console.log("Parameterised constructor of Employee is called");
	this.empId = id;
	this.ename = name;
	this.salary = sal;
};

$__jsx_extend([Employee, Employee$0], Object);
Employee.prototype.showDetails$ = function () {
	console.log("Employee details are " + (this.empId + "") + " " + this.ename + " " + (this.salary + ""));
};
...
function _Main$main$AS(args) {
	...
	var e1;
	numbers = [ 1, 2, 3, 4, 5 ];
	listItem = numbers.map((function (number) {
		return "<li key=" + number.toString() + ">" + ((function (v) {
			if (! (v != null)) {
				debugger;
				throw new Error("[hello.jsx:33:45] null access\n            \"<li key=\"+number.toString()+\">\"+number+\"</li>\"\r\n                                             ^^^^^^\n");
			}
			return v;
		}(number)) + "") + "</li>";
	}));
	...
	e1 = new Employee$0(101, "Eshan", 10000);
	e1.showDetails$();
};
